...
//#endif
  ...
  CopyVector(x, p);
  std::cout << "Test time CG_ref 00 in rank: " << A.geom->rank << " at " << mytimer() << std::endl;
  TICK(); ComputeSPMV_ref(A, p, Ap);  TOCK(t3); // Ap = A*p
  std::cout << "Test time CG_ref 01 in rank: " << A.geom->rank << " at " << mytimer() << std::endl;
  TICK(); ComputeWAXPBY_ref(nrow, 1.0, b, -1.0, Ap, r); TOCK(t2); // r = b - Ax (x stored in p)
  std::cout << "Test time CG_ref 02 in rank: " << A.geom->rank << " at " << mytimer() << std::endl;
  TICK(); ComputeDotProduct_ref(nrow, r, r, normr, t4);  TOCK(t1);
  std::cout << "Test time CG_ref 03 in rank: " << A.geom->rank << " at " << mytimer() << std::endl;
